...
def call(tokensq, boolean bool_1, boolean bool_2) {
    ...
    def Result2 = sh (script: "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=${PROJECT_NAME} -Dsonar.login= ${SONAR_AUTH_TOKEN} -Dsonar.host.url=${SONAR_HOST_URL}", returnStdout: true)
    println Result2
